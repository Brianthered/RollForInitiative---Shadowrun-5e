...
var list = [], // Array to hold players in the game
    ...
    aSkill = 0,
    total = 0,
	actives = [], //array of players with the active class
	activeId; //used to iterate through the players with the active class and clear the active class.
...
function Player(name, baseScore, dice, totalScore, npc, seize, selected) {
    "use strict";
...
//Clear the Active Flag, reset the player text to remove "selected".
function clearActive() {
	var actives = document.getElementsByClassName("active");
	var i;
	for (i=0; i < actives.length; i++) {
		var activeId = actives[i].id;
		document.getElementById(activeId).innerHTML=(list[activeId].name + "<br>" + list[activeId].totalScore);
		document.getElementById(activeId).classList.remove('active');
	}
}

//Set the text on the active player and clear and incorrect text.
function activePlayerText() {
	var selectedPlayerId = whoActive();
	if(selectedPlayerId !== null) {
		showElement(document.getElementById("toolbox"));
		showElement(document.getElementById("removePlayerButton"));
		document.getElementById("removePlayerButton").innerHTML=("Remove " + (list[selectedPlayerId].name));
		document.getElementById(selectedPlayerId).innerHTML = (list[selectedPlayerId].name + "<br>Selected<br>" + list[selectedPlayerId].totalScore);
	}
}
...
function setOptionsList() {
    ...
    if (list[selectedPlayerId].seize) {
    ...
    document.getElementById("owLabel").innerHTML = "Current Score: " + list[selectedPlayerId].overwatch;
}

// Function for when user clicks on a player element in the list.  Sets the active class on the appropriate player id, clears it off of other players.
function choosePlayer() {
	...
	if (document.getElementById(id).classList.contains("active")) {
		...
		// reset text and remove active flag.
		document.getElementById(id).innerHTML=(list[id].name + "<br>" + list[id].totalScore);
		document.getElementById(id).classList.remove("active");
	} else {
		clearActive(); 		// Clear all other active class
		document.getElementById(id).classList.add('active');		// Set the active class to the selected id
		activePlayerText(); 	// Show the toolbox and remove players buttons.  Update player text and remove button.
...
function displayPlayers() {
    ...
    for (index = 0; index < list.length; index += 1) {
		...
		if (list[index].selected == 1) {
			mainLabel.className += " active";
			activePlayerText();
			list[index].selected = 0; // if the js is updated to use the active array element to signify which player is active, this should be removed
		}
...
function sortPlayers() {
	...
	var selectedPlayerId = whoActive();	
	if (selectedPlayerId !== null) {
		list[selectedPlayerId].selected = 1;  // This is all sorts of hacky, and probably the selected array element in the list array should be used everywhere, but currently I am looking at the form and the "active" class.  But I don't want to rewrite this again.  Flag is unset in displayPlayers
	}
...
function addPlayer(form) {
    ...
    if (name === "") {
        ...
        player = new Player(name, base, dice, score, npc, seize, selected);
...
function removePlayer() {
    "use strict";
    var selectedPlayerId=whoActive();
	var selectedPlayer=list[selectedPlayerId];
	clearActive();
	hideElement(document.getElementById("removePlayerButton"));
	hideElement(document.getElementById("toolbox"));
...
function addOptionsButtons() {
    var listItem,
        ...
        selectList,
        newButton,
		selectedPlayerId=whoActive(),
		selectedPlayer=list[selectedPlayerId],
